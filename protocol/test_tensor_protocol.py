...
try:
    ...
    # this is needed since the rust bindings generate a different event loop
...
# just an asyncio wrapper for the uniffi event loop
def setup_event_loop():
    """Setup the event loop for UniFFI async operations."""
    try:
        # Get the current event loop (will be created by asyncio.run())
        loop = asyncio.get_running_loop()
        uniffi_set_event_loop(loop)
        print("✅ Event loop configured for UniFFI")
    except RuntimeError:
        # If no event loop is running, asyncio.run() will create one
        print("⚠️  No event loop running yet - will be set up later")
...
async def test_larger_tensor():
    ...
    try:
        ...
        # we need to wait longer for the chunked tensor to be sent
        await asyncio.sleep(2)  
        
        # Try to receive with more attempts for larger tensor
        received_tensor = None
        for attempt in range(1, 400):  # More attempts for larger data
            received = await node2.receive_tensor()
            if received:
                received_tensor = received
...
async def main():
    ...
    # run the uniffi event loop; without this, things are difficult
    setup_event_loop()
...
if __name__ == "__main__":
    ...
    # # ✅ ADD THIS: Alternative event loop setup for when running directly
    # try:
    #     loop = asyncio.new_event_loop()
    #     asyncio.set_event_loop(loop)
    #     uniffi_set_event_loop(loop)
    #     print("✅ Event loop configured for UniFFI (direct execution)")
    # except Exception as e:
    #     print(f"⚠️  Event loop setup warning: {e}")
